...
- [android/app/src/main/java/com/portai/app/MainActivity.java](file://android/app/src/main/java/com/portai/app/MainActivity.java)
</cite>

## 更新摘要
**所做更改**
- 增强移动端视口高度管理，使用动态 dvh 单位处理非原生平台，为原生平台使用冻结高度解决 iOS Safari 底部导航栏裁剪问题
- 实现平台特定处理逻辑，使用 isNativePlatform() 检查区分不同平台的视口行为差异
- 改进跨平台行为注释，详细说明各平台视口管理的特殊考虑
- 优化 Android 壳的键盘处理和状态栏避让机制
...
**更新** 增强了移动端视口高度管理，针对不同平台采用不同的策略来解决 iOS Safari 底部导航栏裁剪问题。
...
MobileLayout --> NativePlatform["isNativePlatform()"]
NativePlatform --> Capacitor["Capacitor.isNativePlatform()"]
```
...
- [src/platform/capabilities.ts:12-15](file://src/platform/capabilities.ts#L12-L15)
...
- **isNativePlatform**：平台检测函数，用于区分原生平台和 Web 平台

**更新** 新增平台检测功能，用于实现跨平台视口高度管理。
...
participant NP as "isNativePlatform"
U->>DM : 窗口大小/方向变化
DM-->>ML : 设备模式变化
alt 桌面模式
ML->>DL : 渲染桌面布局
...
**更新** 新增平台特定处理逻辑，使用 isNativePlatform() 区分原生平台和 Web 平台。
...
- 处理 Android 返回键事件
- 智能滚动定位避免键盘遮挡
- **平台特定的视口高度管理**：原生平台使用冻结高度，Web 平台使用动态 dvh 单位

**更新** 增强了视口高度管理，针对不同平台采用不同的策略来解决 iOS Safari 底部导航栏裁剪问题。

**章节来源**
- [src/components/layout/MainLayout.tsx:59-285](file://src/components/layout/MainLayout.tsx#L59-L285)

### 视口高度管理详解
MobileLayout 实现了复杂的跨平台视口高度管理策略：

```mermaid
flowchart TD
Start(["组件挂载"]) --> CheckPlatform{"是否原生平台?"}
CheckPlatform --> |是| FreezeHeight["冻结初始 innerHeight"]
CheckPlatform --> |否| UseDvh["使用 100dvh CSS 单位"]
FreezeHeight --> ApplyStyle["应用 frozenHeight 样式"]
UseDvh --> DynamicHeight["动态跟随地址栏变化"]
ApplyStyle --> HandleInsets["处理系统栏 insets"]
DynamicHeight --> HandleInsets
HandleInsets --> End(["完成渲染"])
```

**图表来源**
- [src/components/layout/MainLayout.tsx:170-194](file://src/components/layout/MainLayout.tsx#L170-L194)
- [src/platform/capabilities.ts:12-15](file://src/platform/capabilities.ts#L12-L15)

**平台特定行为说明：**
- **Android 原生平台**：使用冻结高度防止键盘弹起时 dvh 变化导致整页收缩
- **iOS Safari/PWA**：使用动态 dvh 单位跟随地址栏展开收起，避免底部导航栏被裁剪
- **其他 Web 平台**：默认使用 dvh 单位获得最佳兼容性
...
### 原生平台检测（isNativePlatform）
职责
- **平台识别**：检测应用是否运行在 Capacitor 原生壳内
- **跨平台兼容**：为 Web 和原生平台提供统一的检测接口
- **未来扩展**：支持未来的 iOS IPA 和其他原生平台
...
- **MobileLayout** 依赖 isNativePlatform 进行平台检测以实现跨平台视口管理
...
- **平台检测优化**：isNativePlatform() 使用简单的布尔检查，避免重复计算
...
- **移动端手势冲突**
  ...
  - 验证 touchAction 样式配置
- **iOS Safari 底部导航栏裁剪问题**
  - 确认 isNativePlatform() 检测是否正确
  - 检查 dvh 单位的使用场景
  - 验证冻结高度的应用时机

**更新** 新增 iOS Safari 相关问题的排查指南。
...
## 结论
MainLayout 作为应用根布局，通过引入设备模式检测功能和增强的跨平台视口高度管理，实现了真正的跨平台响应式设计。它能够自动识别设备类型并在移动端抽屉导航和桌面端侧边栏导航之间无缝切换，同时针对不同平台采用最优的视口高度管理策略，解决了 iOS Safari 底部导航栏裁剪等关键问题。配合 MobileLayout、DesktopLayout、DesktopSidebar 等组件，形成了完整的多平台布局解决方案。遵循本文的性能建议与最佳实践，可进一步提升用户体验与可维护性。

**更新** 强调了跨平台视口高度管理的重要性及其对用户体验的影响。
...
- **平台特定的视口高度管理**：理解不同平台的视口行为差异，选择合适的处理方式

**更新** 新增平台特定视口管理的最佳实践指导。
...
### 跨平台视口管理最佳实践
- **原生平台**：使用冻结高度防止键盘弹起导致的布局抖动
- **Web 平台**：使用 dvh 单位获得最佳的地址栏跟随效果
- **平台检测**：使用 isNativePlatform() 进行统一的平台判断
- **状态栏处理**：为原生平台提供正确的状态栏避让
- **键盘处理**：正确处理不同平台的键盘行为差异
...
- [src/components/layout/MainLayout.tsx:170-194](file://src/components/layout/MainLayout.tsx#L170-L194)
- [src/platform/capabilities.ts:12-15](file://src/platform/capabilities.ts#L12-L15)
- [android/app/src/main/java/com/portai/app/MainActivity.java:289-324](file://android/app/src/main/java/com/portai/app/MainActivity.java#L289-L324)