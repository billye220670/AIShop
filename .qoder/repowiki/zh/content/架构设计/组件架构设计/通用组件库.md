...
## 更新摘要
**所做更改**
- 新增 BlobImage 组件章节，详细说明其增强的样式支持和 onLoad 回调功能
- 更新 MessageImage 组件章节，说明如何利用 BlobImage 的新特性实现精确尺寸控制
- 增强图片处理组件的依赖关系分析
- 更新使用示例和最佳实践部分
...
- BlobImage：增强的图片显示组件，支持 IndexedDB 图片、自定义占位符、回退内容、加载完成回调，提供 naturalWidth/naturalHeight 访问能力。
...
- 图片处理：BlobImage 提供统一的图片加载体验，支持多种状态处理和回调机制。
...
U->>BI : 加载图片
BI-->>U : onLoad(naturalWidth, naturalHeight)
```
...
- 图片处理：BlobImage 依赖 useBlobUrl hook，MessageImage 依赖 BlobImage 提供的基础能力。
...
## 结论
本组件库以受控状态与内部状态结合的方式，提供了高内聚、低耦合的可复用 UI 能力。通过统一的类型定义与主题变量，确保了跨场景的一致性与可维护性。ModelSelector 的双模式（下拉/底部弹窗）提升了桌面与移动端的体验；ConfirmModal 与 PromptModal 提供了标准的交互范式；CustomSelect 则补齐了原生 select 的样式与行为差异；BlobImage 及其衍生组件 MessageImage 提供了强大的图片处理能力，支持精确的尺寸控制和丰富的加载状态管理。建议在实际使用中遵循本文档的 API 规范与最佳实践，并结合测试策略保障质量。
...
- BlobImage
  - Props: src, alt, className, style, draggable, loading, placeholder, fallback, onLoad
  - 事件: onLoad(event)
...
- 图片显示
  ...
  - 参考: [BlobImage.tsx:25-65](file://src/components/common/BlobImage.tsx#L25-L65), [MessageImage.tsx:21-58](file://src/components/chat/MessageImage.tsx#L21-L58)
