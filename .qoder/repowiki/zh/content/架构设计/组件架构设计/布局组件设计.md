...
章节来源
- [MainLayout.tsx:74-201](file://src/components/layout/MainLayout.tsx#L74-L201)
...
CheckAxis -- 是 --> Move["touchmove"]
Move --> AxisLock{"是否已锁定轴向?"}
...
Flig --> SetState["reset + onOpenChange + onSettle"]
Ratio --> SetState
```
...
## 结论
AIShop 的布局组件以 MainLayout 为核心，结合 Sidebar、TopNavBar、BottomNavBar 形成完整的移动端布局体系。通过 useDrawerSwipe 实现高性能的手势交互，配合 haptics 提供触觉反馈；样式系统基于主题变量与 Tailwind，保证一致性与可维护性。整体架构清晰、职责明确，具备良好的可扩展性与可访问性。建议在新增布局特性时遵循现有模式：将手势与状态下沉至 Hook/工具函数，UI 组件保持纯展示与局部状态管理，并通过 Props 接口进行松耦合通信。
