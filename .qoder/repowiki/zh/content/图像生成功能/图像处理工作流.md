...
- [src/services/imageIntentJudge.ts](file://src/services/imageIntentJudge.ts)
- [src/hooks/useChat.ts](file://src/hooks/useChat.ts)
- [src/services/promptOptimizer.ts](file://src/services/promptOptimizer.ts)
</cite>

## 更新摘要
**变更内容**
- 新增图片意图检测系统章节，详细说明智能判断上传图片的处理策略
- 更新聊天集成流程，展示意图检测与提示词优化的协同工作
- 增强错误处理机制说明，包含意图检测失败的降级策略
- 完善架构总览图，体现新的意图检测层
...
## 简介
本文件系统化梳理 AIShop 的图像处理工作流，覆盖从用户输入到最终图像生成的完整链路：参数校验、文件预处理（压缩/尺寸提取）、API 调用（多模型适配与超时控制）、结果处理（URL/base64 统一抽取）、历史持久化（IndexedDB + Blob 引用计数）、批量任务调度（并发队列）以及错误处理与重试机制。**最新更新**：新增了智能图片意图检测系统，能够自动判断用户意图是进行图片生成还是文本对话，并智能选择处理方式。同时提供存储管理策略、性能优化建议与常见问题排查方法。

## 项目结构
围绕图片生成能力，前端采用"Hook + 服务 + UI"的分层组织：
- 交互层：ImagePanel 负责输入、拖拽上传、瀑布流展示、下载与删除等。
- 状态与流程层：useImage Hook 封装模型选择、参数派生、并发队列、重试/取消、历史读写；useChat 集成意图检测与聊天流程。
- **新增**：意图检测层：imageIntentJudge 智能判断用户意图，决定走图片生成还是文本对话路径。
...
Chat["useChat<br/>聊天流程"] --> Intent["imageIntentJudge<br/>意图检测"]
Intent --> |需要图片| ImageFlow["图片生成流程"]
Intent --> |文本对话| ChatFlow["聊天对话流程"]
...
- [src/services/imageIntentJudge.ts:137-186](file://src/services/imageIntentJudge.ts#L137-L186)
- [src/hooks/useChat.ts:833-970](file://src/hooks/useChat.ts#L833-L970)
...
- **新增**：图片意图检测 imageIntentJudge：使用轻量小模型智能判断用户意图，区分图片生成和文本对话需求，支持编辑上一张 AI 图片的智能识别。
- 聊天集成 useChat：整合意图检测结果，协调提示词优化和图片生成流程。
...
## 架构总览
下图展示了从用户输入到结果展示的端到端流程，包括新增的智能意图检测层、参数校验、并发队列、API 调用、结果处理与持久化。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "ImagePanel/Chat"
participant INT as "imageIntentJudge"
participant H as "useImage/useChat"
participant A as "imageApi"
participant D as "imageHistoryRepo/blobRepo"
participant R as "上游API"
U->>UI : 输入提示词/拖入参考图
UI->>INT : judgeImageIntent(userText, messages, imageCount)
INT-->>UI : {needImage, reply, editPrevious, aspectRatio}
alt needImage = true
...
### 智能图片意图检测系统
**新增功能**：图片意图检测系统通过轻量级小模型智能判断用户意图，实现聊天和图片生成的自动分流。

#### 核心特性
- **智能分类**：准确识别用户是想生成图片还是进行普通对话
- **上下文理解**：分析最近4条消息历史，理解追问和修改意图
- **编辑识别**：智能识别是否要编辑上一张 AI 生成的图片
- **比例推荐**：为文生图任务推荐合适的宽高比
- **容错机制**：判断失败时静默回退到默认行为，不影响主流程

#### 工作流程
```mermaid
flowchart TD
Start(["用户输入"]) --> Extract{"提取文本和图片"}
Extract --> BuildContext{"构建上下文"}
BuildContext --> JudgeModel{"调用意图检测模型"}
JudgeModel --> ParseResult{"解析JSON结果"}
ParseResult --> Decision{"决策分支"}
Decision --> |needImage=true| ImageFlow["图片生成流程"]
Decision --> |needImage=false| ChatFlow["聊天对话流程"]
Decision --> |判断失败| Fallback["默认聊天流程"]
subgraph "编辑场景识别"
EditCheck{"是否编辑上一张AI图?"}
EditCheck --> |是| EditMode["编辑模式"]
EditCheck --> |否| NewGen["新图生成"]
end
subgraph "比例推荐"
RatioSelect{"选择合适比例"}
RatioSelect --> |文生图| AspectRatio["推荐宽高比"]
RatioSelect --> |编辑模式| AutoRatio["保持原图比例"]
end
```

**图表来源**
- [src/services/imageIntentJudge.ts:24-75](file://src/services/imageIntentJudge.ts#L24-L75)
- [src/services/imageIntentJudge.ts:88-110](file://src/services/imageIntentJudge.ts#L88-L110)
- [src/services/imageIntentJudge.ts:137-186](file://src/services/imageIntentJudge.ts#L137-L186)

#### 支持的意图类型
- **文生图**：用户明确要求生成图片，如"画一只猫"、"生成一张海报"
- **图片编辑**：用户上传图片要求修改，如"把这张图调亮一点"
- **编辑上一张AI图**：不带新图但想修改之前生成的图片
- **合并参考**：结合用户新图和上一张AI图进行创作
- **纯文本对话**：日常问答、写作、翻译等非图片需求
...
- **新增**：意图检测降级：判断失败时静默返回 needImage=false，保持原有聊天流程。
...
- [src/services/imageIntentJudge.ts:77-78](file://src/services/imageIntentJudge.ts#L77-L78)
...
### 提示词优化与意图检测协同
**新增功能**：意图检测系统与提示词优化器协同工作，确保图片根据其内容和预期用途得到适当处理。

#### 协同工作流程
1. **意图检测阶段**：使用轻量小模型快速判断用户意图
2. **提示词优化阶段**：根据意图检测结果，使用用户选择的模型优化提示词
3. **参数推导阶段**：基于意图结果确定图片尺寸、比例等参数
4. **执行阶段**：调用相应的图片生成 API

#### 优化策略
- **文生图场景**：根据推荐的宽高比优化构图描述
- **编辑场景**：保持原图比例，专注于修改指令的精确表达
- **合并场景**：明确说明多张图片的关系和使用方式
- **失败回退**：优化失败时静默使用原始用户输入

**章节来源**
- [src/services/promptOptimizer.ts:25-34](file://src/services/promptOptimizer.ts#L25-L34)
- [src/services/promptOptimizer.ts:39-93](file://src/services/promptOptimizer.ts#L39-L93)
- [src/hooks/useChat.ts:931-937](file://src/hooks/useChat.ts#L931-L937)

## 依赖关系分析
- 组件依赖：ImagePanel 依赖 useImage；useImage 依赖 imageApi 与 db 模块；UI 通过 BlobImage 渲染本地/远程图。
- **新增依赖**：useChat 依赖 imageIntentJudge 进行意图判断，依赖 promptOptimizer 进行提示词优化。
...
Chat["useChat"] --> IntentJudge["imageIntentJudge"]
Chat --> PromptOpt["promptOptimizer"]
IntentJudge --> Settings["settingsService"]
PromptOpt --> Settings
...
- **新增**：意图检测性能优化
  - 使用固定便宜模型（doubao-1-5-pro-32k-25015）进行意图判断
  - 限制上下文消息数量为4条，减少请求大小
  - 失败时静默回退，不影响主流程性能
...
- 常见错误
  ...
  - **新增**：意图检测失败：检查 LLM 提供商配置，确认 API Key 有效。
- 调试步骤
  ...
  - **新增**：检查意图检测结果，确认是否正确识别用户意图。
- 恢复操作
  - 重试：点击错误卡片的重试按钮，使用原参数重新发起请求。
  - 清理：清空历史或触发孤儿 Blob 清理，释放存储空间。
  - **新增**：意图检测降级：如果意图检测持续失败，系统将自动回退到默认聊天流程。
...
## 结论
AIShop 的图像处理工作流通过清晰的分层设计与完善的错误处理机制，实现了从用户输入到图像生成、持久化与展示的完整闭环。**最新改进**：新增的智能图片意图检测系统显著提升了用户体验，能够准确识别用户的真实意图，自动分流到图片生成或文本对话流程。其亮点包括：
- 多模型适配与灵活的参数派生，兼顾 GPT 与 Gemini 的差异。
- **新增**：智能意图检测系统，实现聊天与生图的自动分流，提升用户体验。
- 基于 IndexedDB 的内容寻址与引用计数，有效管理本地存储与内存。
- 并发队列与超时控制，保障批量任务的稳定执行。
- 丰富的降级策略与用户体验优化，提升鲁棒性与可用性。
- **新增**：提示词优化与意图检测的协同工作，确保图片生成质量。
...
## 附录
- 关键类型定义：ImageGenerationParams、ImageHistoryItem、PendingImageTask、ImageIntentResult 等见 types 文件。
- 模型配置：IMAGE_MODELS 定义了可用的图片模型及其元信息。
- **新增**：意图检测配置：JUDGE_MODEL 使用 doubao-1-5-pro-32k-25015 进行轻量级意图判断。

**章节来源**
- [src/types/index.ts:177-214](file://src/types/index.ts#L177-L214)
- [src/config/models.ts:237-271](file://src/config/models.ts#L237-L271)
- [src/services/imageIntentJudge.ts:15-22](file://src/services/imageIntentJudge.ts#L15-L22)