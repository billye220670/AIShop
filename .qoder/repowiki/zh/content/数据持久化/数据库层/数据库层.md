...
## 更新摘要
**变更内容**
- 新增角色仓库（roleRepo.ts），提供完整的 CRUD 操作支持用户自定义系统提示词预设
- 实现自动名称提取功能，从系统提示词首行智能生成角色名称
- 添加 by_createdAt 索引优化角色列表查询性能
- 集成 BYOC 增量同步系统，支持角色数据跨设备同步
- 扩展数据库 schema，新增 roles 对象存储和索引定义
...
- 数据库连接与升级、写入串行化与错误恢复
- 表结构与索引设计（会话、消息、上下文、检索、二进制、资产、图片历史、**角色**）
- 各仓库的 CRUD 封装与职责边界
- 序列号生成策略与插入排序保证
- **新增** 角色仓库的完整功能：CRUD 操作、自动名称提取、高效查询
...
- schema.ts：类型定义与 DBSchema，**新增** roles 对象存储和 SyncMeta 字段
- seq.ts：浮点排序键生成器
- index.ts：对外统一导出，**新增** 角色仓库 API
- 各 repo：conversationRepo、messageRepo、contextRepo、retrievalRepo、blobRepo、imageHistoryRepo、assetRepo、**roleRepo**（favoriteRepo 保留为兼容，仅 BYOC 镜像读取）
...
B --> L["assetRepo.ts<br/>我的库资产"]
B --> M["roleRepo.ts<br/>角色管理"]
B --> N["messageCodec.ts<br/>编解码器"]
F --> C
G --> C
H --> C
I --> C
...
- [src/db/open.ts:1-139](file://src/db/open.ts#L1-L139)
- [src/db/schema.ts:1-305](file://src/db/schema.ts#L1-L305)

## 核心组件
- 连接与升级：open.ts 提供 getDB/closeDB/withDB/enqueue，集中处理 Safari 内存压力导致的 UnknownError 重连、多标签页升级阻塞处理、按 key 的写操作串行化。
- 数据模型：schema.ts 定义所有实体与索引，约定原文不可变、seq 为浮点数、**新增** updatedAt/syncedAt 用于云同步，**新增** StoredRole 角色类型。
...
participant Role as "roleRepo.createRole"
participant Open as "open.withDB/enqueue"
participant IDB as "IndexedDB"
participant Ret as "BYOC同步"
UI->>Role : createRole(systemPrompt)
Role->>Role : 自动提取角色名称
Role->>Open : enqueue('roles', task)
Open->>IDB : 获取连接/事务
Role->>IDB : put('roles', rec)
...
- **新增** 角色索引：roles 对象存储创建 by_createdAt 索引，支持按创建时间高效查询。
...
- **新增** 资产 assets：key=id；索引 by_createdAt、by_kind；StoredAsset 继承 SyncMeta，承载 artifact/markdown/图片三类用户保存内容（v2→v3 升级时从 favoriteArtifacts 一次性迁移，旧 store 保留不删）。
...
### 资产仓库（assetRepo.ts）
- 功能：listAssets/saveArtifact/saveMarkdown/saveImageHistory/removeAsset/renameAsset/findMarkdownBySourceRef，替代原收藏仓库管理「我的库」资产。
- 三种 kind 统一建模：artifact（含缩略图 blob）、markdown（纯文本内联 + sourceRef 去重）、image（blobIds 沿用 imageHistory 约定）。
- 去重按 kind 区分：artifact/image 以 id 去重（重复保存无操作），markdown 以 sourceRef（源消息 id）去重；findMarkdownBySourceRef 供保存前查重（跳过标题模型调用）。
- 缩略图/图片：data URL 统一落库为 blob 引用；http 上游链接原样保留；删除时释放引用。
- 同步：StoredAsset 继承 SyncMeta，随 BYOC 增量同步（云端 assets 目录为主，favs 目录保留为 artifact 兼容镜像）。
- 标题：markdown 落库标题由 useAssets 统一调用 titleGenerator.generateDocumentTitle 小模型总结（无 key/失败回退内容截取）。

**章节来源**
- [src/db/assetRepo.ts:1-217](file://src/db/assetRepo.ts#L1-L217)
...
- 推送流程：pushLocal 检测本地变更，上传会话、消息、blob、节点、**角色**
...
- 仓库间依赖：
  ...
  - blobRepo 独立，被多个仓库复用。
- **新增** BYOC同步依赖：
  - incrementalSync 依赖所有仓库进行数据同步，包括新的 roleRepo。
...
## 结论
AIShop 的数据层以 IndexedDB 为基础，通过仓库模式清晰划分职责，结合序列化写入、内容寻址存储与索引化检索，实现了高性能、可扩展且易于维护的本地数据方案。**新增的角色仓库**为用户提供了强大的自定义系统提示词预设功能，支持自动名称提取、高效查询和完整的 CRUD 操作。**新增的 BYOC同步系统**通过增量同步和墓碑跟踪机制，确保了多设备间的数据一致性和可靠性。其设计兼顾了流式输出的稳定性、长会话的性能、云同步的可扩展性以及删除操作的跨设备传播能力。
...
**图表来源**
- [src/db/schema.ts:94-305](file://src/db/schema.ts#L94-L305)
