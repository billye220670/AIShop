...
subgraph "UI 层"
G["useBlobUrl.ts<br/>按需创建/释放 object URL"]
...
- [useBlobUrl.ts:1-120](file://src/hooks/useBlobUrl.ts#L1-L120)
...
- [blobRepo.ts:131-167](file://src/db/blobRepo.ts#L131-L167)
- [useBlobUrl.ts:19-61](file://src/hooks/useBlobUrl.ts#L19-L61)

## 架构总览
下图展示了从 UI 到持久化的调用链路与数据流向，重点体现 Blob 引用 URL 的生成、解析与对象 URL 的按需创建与释放。

```mermaid
sequenceDiagram
participant UI as "UI 组件"
participant Hook as "useBlobUrl.ts"
participant Codec as "messageCodec.ts"
...
Hook-->>UI : 返回可渲染 URL
UI->>Hook : 组件卸载
Hook->>Hook : URL.revokeObjectURL(url)
```

图表来源
- [useBlobUrl.ts:23-61](file://src/hooks/useBlobUrl.ts#L23-L61)
...
Hook-->>Comp : 返回 url
Comp->>Hook : 组件卸载
Hook->>Cache : refs--, 若为0则 revokeObjectURL
```
...
章节来源
- [DataSettings.tsx:31-41](file://src/components/settings/DataSettings.tsx#L31-L41)
- [blobRepo.ts:151-167](file://src/db/blobRepo.ts#L151-L167)
...
图表来源
- [open.ts:76-115](file://src/db/open.ts#L76-L115)
...
- 对象 URL 泄漏：确保组件卸载时调用 release，避免长期持有导致内存增长。
...
- [useBlobUrl.ts:52-61](file://src/hooks/useBlobUrl.ts#L52-L61)
- [blobRepo.ts:131-147](file://src/db/blobRepo.ts#L131-L147)

## 结论
该二进制仓库通过内容寻址与引用计数实现了高效、安全的本地 Blob 管理。结合 UI 层的懒加载与缓存机制，有效控制了内存与存储占用。提供统计与清理能力，便于用户感知与维护。对于大文件与流式处理，当前实现以整块 Blob 为主，可在未来引入分块上传与流式写入以提升体验。
