...
- 增量同步层：以 manifest.json 作为"目录"，按 updatedAt/syncedAt 判断变更，使用 tombstones 传播删除。**现已全面支持角色数据的同步，包括自动变更检测和删除传播**。
...
- 最近同步时间：供 UI 展示。
- **增强的待同步计数**：**countPending() 函数现在包含角色待同步数量的统计**，综合 updatedAt/syncedAt 与 tombstone 计算，支撑 60 秒轮询。
