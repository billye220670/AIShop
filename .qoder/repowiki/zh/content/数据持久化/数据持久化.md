...
- 两个生成入口，共用固定模型（豆包 doubao-1-5-pro-32k）与清洗/降级逻辑：
  - generateTitle：会话标题——构造前几轮对话摘要（限长），无 API Key 或失败时回退首条用户消息截取。
  - generateDocumentTitle：文档标题——「我的库」markdown 资产保存时调用，内容截断 500 字，失败时回退内容截取前 20 字。
- 算法流程：
  - 构造摘要：取内容限制总长度。
...
- 连接与升级（open.ts）：
  ...
  - 定义多表结构与索引：conversations、messages、blobs、contextNodes、contextPlans、retrieval、imageHistory、assets、favoriteArtifacts（兼容保留）、roles、kv。
