# PortAI APK 自动打包安装脚本（无交互，无需确认）
# 流程：检查设备 → 构建 Web 产物 → 同步到 Android 工程 → Gradle 编译 APK → 安装 → 自动启动应用
# 用法：
#   默认       全流程（自动安装并启动）
#   -SkipBuild 跳过打包，仅安装现有 APK 并启动
...
Write-Host '  PortAI APK 自动打包安装（无确认）' -ForegroundColor Cyan
Write-Host '======================================' -ForegroundColor Cyan
...
# ---------- 5. 自动启动应用（无需确认） ----------
Write-Host '正在启动 PortAI...'
& $adb -s $deviceId shell am start -n com.portai.app/.MainActivity | Out-Null
Write-Host '[成功] PortAI 已启动。' -ForegroundColor Green
